...
def main():
    ...
    num_data = 10000#10000#1000000
    crop_height = 128
    crop_width = crop_height

    topic = 'animal_texture'
    iPath = '%s/test_resize512' % topic
    oPath = '%s/test_resize512_aug_crop_%d' % (topic, crop_height)
    if not os.path.isdir(oPath): os.mkdir(oPath)

    data = sorted(glob(os.path.join(iPath, '*.png')))
    num_images = len(data)
    num_repeats = num_data // num_images + 1

    pivot = 0
    #pivot = 865//32 *32
    for (num, iFile) in enumerate(data[pivot:]):
    #for (num, iFile) in enumerate(data[pivot:pivot+865//32]):
        idx1 = iFile.rfind('/')
        idx2 = iFile.find('.png')
        name = iFile[idx1+1:idx2]
        for count in range(num_repeats):
            oFile = '%s/%s_aug%08d.png' % (oPath, name, count)
            ...
            print('%d:%d, %d:%d' % (num_images, num+pivot, num_repeats, count))
