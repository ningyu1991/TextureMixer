...
def extract(tfrecord_dir, output_dir):
    ...
    while True:
        ...
        try:
            images, labels = dset.get_minibatch_np(1, lod=1)
        except tf.errors.OutOfRangeError:
            break
        if images.shape[1] == 1:
...
def create_lsun(tfrecord_dir, lmdb_dir, resolution=256, max_images=None, white_background=0, Gaussian_down=0):
    ...
    with lmdb.open(lmdb_dir, readonly=True).begin(write=False) as txn:
        ...
        with TFRecordExporter(tfrecord_dir, max_images, Gaussian_down=Gaussian_down) as tfr:
            for idx, (key, value) in enumerate(txn.cursor()):
                try:
                    ...
                    if white_background:
                        if np.sum(np.prod(img >= 230, axis=0)) >= resolution**2 * 0.7:
                            tfr.add_image(img)
                    else:
                        tfr.add_image(img)
                except:
...
def create_from_images(tfrecord_dir, image_dir, resolution=256, shuffle=1, export_labels=0, left_half=0, right_half=0, Gaussian_down=0):
    ...
    with TFRecordExporter(tfrecord_dir, len(image_filenames), Gaussian_down=Gaussian_down) as tfr:
...
def create_from_hdf5(tfrecord_dir, hdf5_filename, resolution=256, shuffle=1, Gaussian_down=0):
    ...
    with h5py.File(hdf5_filename, 'r') as hdf5_file:
        ...
        hdf5_data = max([value for key, value in hdf5_file.items() if key.startswith('imgs')], key=lambda lod: lod.shape[3]) # specify to iGan dataset wiht key name "imgs" instead of "data"
        with TFRecordExporter(tfrecord_dir, hdf5_data.shape[0], Gaussian_down=Gaussian_down) as tfr:
            ...
            for idx in range(order.size):
                ...
                img = np.asarray(img)
                #tfr.add_image(img)
                tfr.add_image(np.transpose(img, axes=[2,0,1])) # specify to iGan dataset, which transpose the image size from [128,128,3] to [3,128,128]
...
def execute_cmdline(argv):
    ...
    p.add_argument(     '--white_background',help='Only choose images with white background (default: 0)', type=int, default=0)
    p.add_argument(     '--Gaussian_down',  help='Use Gaussian kernel filter before downsampling rather than mean value downsampling (default: 0)', type=int, default=0)
    ...
    p.add_argument(     '--export_labels',  help='Create -rxx.labels alongside the tfrecords (default: 0)', type=int, default=0)
    p.add_argument(     '--left_half',      help='Only crop out the left half images (default: 0), specifically for the pix2pix facades/maps dataset', type=int, default=0)
    p.add_argument(     '--right_half',     help='Only crop out the right half images (default: 0), specifically for the pix2pix maps dataset', type=int, default=0)
    p.add_argument(     '--Gaussian_down',  help='Use Gaussian kernel filter before downsampling rather than mean value downsampling (default: 0)', type=int, default=0)
